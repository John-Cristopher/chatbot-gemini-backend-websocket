...
def get_user_chat():
    ...
    if session_id in active_chats and active_chats[session_id] is None:
        print(
            f"Recriando chat Gemini para session_id existente (estava None): {session_id}"
        )
        try:
            chat_session = client.chats.create(
                model=MODELO,
            ...
            app.logger.error(
                f"Erro ao recriar chat Gemini para {session_id}: {e}", exc_info=True
            )
            raise
...
def handle_connect():
    ...
    try:
        ...
        emit(
            "status_conexao",
            {"data": "Conectado com sucesso!", "session_id": user_session_id},
        )
    except Exception as e:
        app.logger.error(
            f"Erro durante o evento connect para {request.sid}: {e}", exc_info=True
        )
...
def handle_enviar_mensagem(data):
    ...
    try:
        ...
        app.logger.info(
            f"Mensagem recebida de {session.get('session_id', request.sid)}: {mensagem_usuario}"
        )
        ...
        app.logger.info(
            f"Resposta enviada para {session.get('session_id', request.sid)}: {resposta_texto}"
        )
